...
function signUp(){
    ...
    if ($("#nombre").val()=="" || $("#lastname").val()=="" || $("#correo").val()=="" || $("#edad").val()=="" || $("#codigo_postal").val()=="" )
    {
        var opts={
        ...
        .afui.toast(opts);
            ...
            //Sexo escrito como Femenino o Masculino
    else if (!($("#sexo").val()=="Femenino" || $("#sexo").val()=="femenino" || $("#sexo").val()=="Masculino" || $("#sexo").val()=="masculino")) 
    {
        var opts={
            message:"Escribe tu sexo como 'Femenino' ó 'Masculino'",
            position:"tc",
            delay:2000,
            autoClose:true,
            type:"error"
		...
		$(data).each(function (index, data) {   
			$("#platillos").append(
				"<li>"+
					...
					"<a href='#page6' id="+data.id_platillo+"><img src="+data.imagen+"/></a>"+
					"</div>"+
				"</li>");
		});	
...
//RESTAURANTES
	$.getJSON('/users/restaurante',function(data){
	console.log(JSON.stringify(data));
						
		$(data).each(function (index, data) {   
			$("#restaurantes").append(
				"<li class='lista_dinero'  onclick='res_platillos(id_restaurante"+data.id_restaurante+");'>"+
                    "<div class='grid-photo-box img-restaurant'>"+
                    	"<p id='id_restaurante"+data.id_restaurante+"'>"+data.id_restaurante+"</p>"+
                        "<a href='#item_restaurante'><img src="+data.imagen+"/></a>"+
                    "</div>"+
                    "<div class='swipe-content'>"+
                        "<a href='#item_restaurante'><h3>"+data.nombre+"</h3></a>"+
                    "</div>"+
                "</li>");
		});	
	});
